const getMarkTracker = () => {
  ...
  return (mark) => {
    ...
    if (prevMark !== null) {
      ...
      duration = performance.getEntriesByName(measureMark, "measure")[0]
        .duration;
...
(async () => {
  ...
  const items = dimension ** 2;

  const matrix = [...Array(items).keys()];
  ...
  for (let i = 0; i < 5; i++) {
    ...
    console.log("result size " + new Float32Array(arrayBuffer)[0]);
    durations.push(perf("postMap" + i));
  }

  const sum = durations.reduce((agg, dur) => agg + dur, 0);
  const mean = sum / durations.length;
  const squaredDistancesSum = durations
    .map((dur) => (dur - mean) ** 2)
    .reduce((agg, dur) => agg + dur, 0);
  const variance = squaredDistancesSum / durations.length;
